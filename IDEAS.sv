...
- **Tables** — `{sql}` finally exercises reference-never-embed; DuckDB as the engine (parquet/CSV/SQL, 100k+ rows). **Measured 2026-08-10: the duckdb *CLI* can be the whole engine with no wrapper** — 11ms cold start, 37ms for a million-row aggregate, `-markdown` straight into the prose path, `-jsonlines` streaming from 11ms for the big case, `-readonly` enforcing immutability (numbers and consequences on V3). Grid verdicts recorded 2026-08-08: VisActor VTable first (pure-JS canvas, UMD-vendorable, framework-neutral, async data model), AntV S2 runner-up, Glide disqualified (React-only) — and embedding a live sqlnow session may beat a native block for the explore case. *(V2.sv candidates; DESIGN.md; HANDOFF)* **→ v3 (the plan)**
