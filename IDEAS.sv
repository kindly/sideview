...
- **Tables** — settled 2026-08-10 (V3's plugins section): the block is the **big-table case only**, a windowed grid over a stdio protocol, with sqlnow supplying grid and engine and **no duckdb binary required**; small tables are an agent piping markdown from any CLI into a prose block. Superseded grid verdicts, kept so they aren't re-derived (2026-08-08): VisActor VTable first (pure-JS canvas, UMD-vendorable, framework-neutral, async data model), AntV S2 runner-up, Glide disqualified (React-only) — and embedding a live sqlnow session may beat a native block for the explore case. *(V2.sv candidates; DESIGN.md; HANDOFF)* **→ v3 (the plan)**
...
- **Real navigation: page loads per path, liveness kept per page** (author's question, 2026-08-18: why is this a SPA?). The honest split: the *page* must be a long-lived client — in-place patching is the product — but the *router* accreted from V0's URL-less switcher and was never chosen. An MPA cannot have the bugs the client router keeps producing (thread 28's cross-page misfile, the wrong-category strip, the dead back button), and it is architecturally cheap here because every connection is already a designed full-state resend. Casualties: instant chip switching, and auto-follow — the one feature that needs a client deciding to move, whose pinning saga suggests it costs what it earns. Trigger: the next navigation-state bug, not a date. **The trigger fired the same day it was filed** (back still broke — comments held across pages, layout mucked) **and it is built, 2026-08-18**: the client router is deleted (no pushState, no popstate, no auto-follow, no cross-page state), every chip and index entry is a real `<a>`, and `/` is a server 302 to the most recently active page — the URL simply *is* what auto-follow used to compute. Back, middle-click and bookmarks work because nothing intercepts them.
