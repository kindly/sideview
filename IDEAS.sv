<sv-page label="ideas: the pool v4 draws from" category="plan" order="2">

<sv-prose id="intro">
# The pool: everything discussed, still unbuilt

Every idea still standing after three shipped versions, in one place. v4 was curated 2026-08-20 through the vision grill: [VISION.sv](VISION.sv) holds the north-star this pool is ranked against, and V4.sv took its In list (ask block, prose editing from the page, restart + store identity, `_sv_cell` marks) out of here. One line per idea; the reasoning lives in the named doc, and reopening anything deferred twice stays a deliberate act (V2's rule). Comment on a bit to argue it in or out; survivors move to the next version's In list at each curation.
</sv-prose>
...
- **Patch-in-place block rendering**: idiomorph-style morphing instead of wholesale replacement; restores native scroll anchoring (the client-side reading anchor covers the symptom today). v3's imported formats raised its price: a one-block markdown page re-renders whole on every save. *(V2.sv candidates; V3.sv formats)*
...
- **A wayfinder ritual skill**: mattpocock/skills' wayfinder (multi-session planning as a map of decision tickets on an issue tracker, MIT) adapted to sideview — the map a committed page (destination and decisions canon, arguing in threads), grilling tickets as ask rounds, research tickets as dispatched agents landing blocks, prototypes embedded live, ticket claiming via `watch --claim`'s existing exactly-once semantics (its first real workload). Route: personal skill first, run on a genuinely oversized chunk of work (v5, a GEM project); ships as the second ritual skill only after `sideview-grill` proves the multi-skill install. *(chat, 2026-08-20)*
