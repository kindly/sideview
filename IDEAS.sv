...
Every idea still standing after five shipped versions, in one place — the continuing pool, no longer tied to any version's curation. [VISION.sv](VISION.sv) holds the north-star this pool is ranked against. v4 drew its In list from here (2026-08-20, through the vision grill); v5 deliberately drew nothing — it is structural work, curated 2026-08-23 through its own simplification grill ([V5.sv](V5.sv)); v6 (curated 2026-08-26 on [V6.sv](V6.sv), round 1) draws from the trust/sharing cluster — a funnel-shared link-plus-name variant of T2. One line per idea; the reasoning lives in the named doc, and reopening anything deferred twice stays a deliberate act (V2's rule). Comment on a bit to argue it in or out; survivors move to a version's In list at each curation.
</sv-prose>
...
- **Reconnect replay morphs too**: steady-state SSE patches morph in place since v5, but replay after a reconnect still clears and rebuilds, so the scroll-preservation machinery survives instead of shrinking; mark-and-sweep against ghost blocks is the remaining work. *(V5 round 5, thread 130)*
...
- **T2, live read-only over the tailnet**: `tailscale serve`, `Tailscale-User-Login` gives comments real authors; read-only enforced server-side by role (Voila's rule). **Bent into v6 (2026-08-26)**: capability links + self-declared names over the *funnel*, guests commenting rather than read-only; the tailnet-identity form stays here for later — headers compose with links. *(SHARING.md; V6.sv)*
...
- **Revocation severs live SSE streams**: v6 checks access at connect, so a revoked guest's already-open tab keeps receiving its page's updates until reconnect (every new request is refused — that line held). Found live during v6's done-when session; accepted deliberately (round 8: a stream is at most one tab-lifetime of already-granted reading). The fix is a re-check per event or a generation-stamped connection registry. *(V6.sv done-when; thread 148)*
- **Claude Code Channels as a watch transport**: the harness's documented replacement for persistent monitors — an external system pushes events into a running session over WebSocket, nothing polls or re-arms. sideview's daemon already emits the event stream, so a `sideview monitor claude` speaking that protocol would retire the wake-on-exit re-arm loop (0.6.2's `watch --once`) on Claude Code the way `monitor codex` retired the shell watcher on Codex. Needs the protocol read first. *(chat, 2026-09-25; HANDOFF 0.6.2)*
- **Foreign-proxy publishing (exe.dev-shaped)**: for a box that can't run tailscale (shared with teammates), the v6 gate is blind — it keys on the funnel mark, so a foreign edge's traffic looks local and the daemon is naked. Needs: a **gate-everything mode** (every HTTP request requires a token, loopback included — costs nothing locally since the CLI writes files/db, never HTTP; round 1's rejected all-requests fork returning as the right answer for foreign proxies), `share` minting against a configured `public_url` instead of driving the funnel, and — only if host-based routing is impossible — `--base-path` (the standard behind-nginx ask; sideview's absolute URLs are prefix-unsafe today). Deferred 2026-09-08: the tagged-node + destination-only-ACL route costs zero code and covers the shared-box case. *(chat, 2026-09-08)*
