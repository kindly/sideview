...
- **The agent authors through a CLI over a local file.** No HTTP, no socket, no library
  ...
  sockets, which hunk and herdr both hit, so anything network-shaped forces every agent
...
Design every block type so that its expensive part sits on the far side of a
...
This rule also tests a proposed block type: if the agent has to read the data
to display it, the block is designed wrong.
...
> and document pages in the repo, with promotion a `mv` instead of an UPDATE.

**The scratch stream** is a per-project visual scrollback. `sideview show <thing>` appends a
view and it appears immediately: a parquet file, an image, a diff, a command's output. No
plan, no title, no ordering decisions. This is the visual equivalent of `less`. It earns
daily use, and its ergonomics matter more than anything else in the CLI.
One short command, sensible type detection from the argument, no setup.

**Plans** are named, ordered, durable documents with prose between the views. A plan is a
...
session, so there's no need for each worktree to carry its own store. A worktree's
...
by walking up like `.git`, holding many sessions. Keeping the *file* at project level
preserves what made project scope attractive: blocks can use relative paths, `pack`
...
`ssh -L`, one bookmark. That argument dissolves over a tailnet, where the host is directly
addressable and no forwarding step exists. **Per session, from a pool of pre-started daemons**,
gave structural single-window isolation but made you decide in advance how many sessions you'd
run, and guessing low returned the prompt the whole scheme existed to avoid. Everything stays
inside the project, nothing under `$HOME`, since the sandbox's write allowlist covers the
working directory and not the home directory. See [V0.md](V0.md) for the details.
...
rather than sitting in the database, which keeps those transactions short.
...
That surfaces the workflow this actually implies: you show yourself things while working,
then keep the ones that turned out to matter. So `sideview promote <view> --to <plan>` is how
plans get built in practice, assembled from evidence you already looked at rather than
authored from a blank page. That's a better story than the one the design started with.

### Identifying the session

> **Superseded by [V0.md](V0.md)'s "Which session am I?".** The chain below includes the controlling
> tty; V0 removes it with reasons, since agent Bash calls have no tty and each is a fresh shell.
> V0 also names the harness variable (`$CLAUDE_CODE_SESSION_ID`) rather than leaving it abstract.
> Build from V0.
...
about chattiness, twenty blocks meaning twenty tool calls, was misplaced: an agent puts twenty
...
That keeps the database small and write transactions short, which keeps one daemon
...
> a headline feature. The section's argument held for what it was looking at, app
> blocks and live query results, and simply predates prose-and-structure becoming a file. Its
...
the case CSV is good at; the escaping that would make it a bad *authoring* format
...
terminal as a fenced block, each labelled with what produced it. That makes the export
useful as a record rather than a page full of `[interactive block]` holes, and it
...
the whole review workflow around here is built on `git diff` and untracked files. A
...
on a second machine is a strong property for a document whose job is to be
believed.
...
- **`dated`**: inputs are identified but mutable, a live Postgres, an API. Record what can
  be recorded: connection target, a declared fingerprint query, the timestamp. Be
...
A reader deciding how much weight to put on a number is better served by an honest
grade than by a hash that covers only some of the inputs.
...
- **Redact.** The diff is source code, and a recorded command line can contain a credential.
  ...
  block opt out of diff capture. Provenance travels inside snapshots and packs, so it is a
  disclosure risk; see [SHARING.md](SHARING.md).
...
show a working prototype in any language. The design has to treat them as first-class
...
the daemon. Anything language-specific is a small script the agent writes *in the project's
own language*: a driver that calls one function, a harness that exercises one module.
Sideview only ever runs it and renders its streams.
...
- **Ports and proxying.** The daemon allocates a port per service block, supervises the
  ...
  subpath is notoriously fiddly: absolute URLs, cookies, websockets. Prefer
  per-block origins (a port or a wildcard localhost subdomain) and iframe by origin, rather
  than rewriting someone's routing.
- **Teardown validates the lifecycle decision.** When the agent dies, the daemon dies, and
  every supervised child dies with it. No orphaned dev servers, no leaked containers. That
  falls out of the daemon model already chosen.
...
- **Snapshots are a real problem.** A live app cannot be frozen into HTML. The honest answer
  is a headless still capture plus the block's config, clearly marked as a photograph. That
  means T0 sharing degrades exactly where the plan is most interesting; worth knowing up
...
This is what makes sideview worth using on day one, before any service block
...
today, and it stays as the escape hatch for bespoke visuals.
...
hand-writing at rung 2 is the signal for what should become a rung-3 block, which
...
That reasoning imported a public-web threat model into a situation that doesn't have
...
2. **The authority it would borrow is removed server-side.** The server enforces read-only
   sharing on the session's *role*, not by inspecting block content: it accepts no execute
   requests from the front end, which is Voila's rule. A read-only session has no
   process-running authority for script to abuse. Stripping scripts was redundant with a
...
- **A `connect-src 'self'` CSP on shared pages** closes the accidental-exfiltration path for
  ...
  third-party hosts. Cheap, non-blocking, worth having, but a hygiene measure rather than
...
Disclosure and lifetime address both: blocks declare what they run before
...
The caveat: Rust is slower to write, and the browser-side TypeScript is identical
...
cron job with no tty at all. That last case is the clean test of the design: a scheduled
...
- `tailscale serve` is nicer: a stable HTTPS URL plus `Tailscale-User-Login` identity, and
...
The aim in [README](README.md), indistinguishable from opening an HTML file with no second
...
under 50ms of added latency, and the browser launch, 100ms warm and seconds cold, is identical
to the HTML-file case. The dominant cost isn't ours, which is why the target is reachable.

**The CLI must return immediately.** Fire the browser open and don't wait on it. A window
must never stall the agent.
...
**Install is one file.** A single static binary, the Rust decision paying for itself: no
interpreter, no venv, no `node_modules`, nothing to resolve against the project's own
dependencies.
...
the whole pipeline end to end, with almost no block-specific machinery. CLI writes a row,
daemon notices, page patches in place, iframe sandbox and sizing work.
