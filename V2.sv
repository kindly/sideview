...
- **Conversation is two tables, split by what the data is (author, 2026-08-07).** A `threads` table owns *placement*: page, target, anchor, the `quote`/`context` captured at creation, and resolution state. Placement is a property of the conversation, not of each utterance: re-resolution after churn updates one thread row, not N, and a reply written days later never re-captures a quote. A `comments` table owns *utterances*: thread id, body, `author`, created_at, and `seen_at` as a **claim marker**, not a read marker. Author is **role before identity (author, 2026-08-07): `agent` when written through the CLI, `user` when written from the page**; SHARING T2's tailnet identity later refines the user side only. The role also lets a watcher skip the agent's own echoes, and lets the bar mark whose turn it is: a thread whose last word is the agent's wears an "agent replied" marker, the user's move. Both are multi-writer and bursty, which is SQLite's job. `page rm` cascades threads, and their comments with them.
...
placement went from margin-marks to trailing bubbles to the bar, each
revision requested through the previous design; creation went from hover to
double-click to selection to the corner chip, chased through the OS's own
selection UI; author became a role, resolution became etiquette, drafts
became plural, the bar learned to never close itself, and the silence got
its ladder of sent, working…, replied, resolved, the final rung landing
minutes before this sign-off. Every word above was earned on this page.
</sv-note>

<sv-prose id="watch">
## `sideview watch`, the agent's await

- A blocking read on the store itself: polls a **generation counter the store bumps in the same transaction as every conversation write** (author, 2026-08-08). One O(1) row read, and the atomicity is the contract: "gen moved" and "the data is visible" are one fact. The lineage, kept so it isn't relitigated: `data_version` was the design until it missed a cross-process commit under WAL after a long idle (live, 2026-08-08: a comment sat invisible until an unrelated write shook it loose); an aggregate probe stood in briefly; a queue table of undelivered items was considered and rejected, because watchers are readers of one shared history (claim already arbitrates who *acts*), not consumers of per-watcher queues. Events print as they arrive; `--timeout N` gives up quietly. `--ack` (author's ask, 2026-08-09, closing the loop's last silence) stamps each emitted comment's `seen_at` as a **delivery receipt**, which the bar shows as **"sent"**. Honest wording, the author's correction: it proves the server has it, not that an agent read it. For work that will take a while, `sideview working <thread>` sets the thread's working marker, an explicit agent call, spent only when worth it, shown pulsing as **"working…"** and retired automatically by the agent's next reply or a resolve. The silence ladder: sent (plumbing, free), then working… (agent, optional), then agent replied, then resolved. **Typed JSON-lines from day one**, `comment`, `resolve`, `unresolve`, so future event kinds don't break consumers; a resolve is an event in its own right, since it is often the "feedback addressed, move on" signal the agent is actually waiting for. Watch holds its own cursor, and `--claim` uses the supersession pattern (`UPDATE … WHERE seen_at IS NULL RETURNING`) for exactly-once when several agents serve one page. A watcher that joins late reaches back with `--since 0 --claim`. The claim marker is what makes reach-back safe, and it should be the standing form (learned live, 2026-08-07: three pre-watcher comments sat invisible until a db query surfaced them).
- Sandbox-compatible (SQLite file access, no network) and daemon-independent: it works no matter who started what.
...
- Nothing about storage changes: all pages are files already, and the db only holds the binding (the daemon's watch list). Committed pages work today, and this very file is served through the ordinary machinery, but its binding had to be a hand-written SQL INSERT. The feature is the missing verb.
...
One JSON object per line on stdout; `watch` starts from its invocation moment (`--since <id>` to reach back), and `--claim` marks `seen_at`/`seen_by` via `UPDATE … WHERE seen_at IS NULL RETURNING` so concurrent watchers get exactly-once.
...
sideview working <thread>                                # "this will take a while" — reply retires it
sideview watch [--timeout N] [--since ID] [--claim] [--skip-author <role>] [--ack]
...
**An MCP route**, reopened by the author (2026-08-07) after the stateless 2026-07-28 spec deleted what the original rejection objected to (stateful stdio servers, handshakes, session ids, per-client lifecycle). The shape when built: the daemon grows `/mcp` as a fourth face of the same binary; tools map 1:1 onto CLI verbs through the same internals and locks; page targeting is an explicit tool parameter (stateless has no session, which is honest anyway); `watch` becomes a Multi-Round-Trip `await_comments`. Buys hosted no-shell agents over the tailnet, structured args, self-describing tools. Two invariants: the CLI stays primary (it works daemon-down; MCP doesn't), and `/mcp` is the first network *authoring* channel, a deliberate trust-line crossing, gated (loopback/token/tailnet identity), never a default.

**Watched diffs and the git machinery**, re-sequenced out of v2 by the author (2026-08-07): proven possible, mechanism settled (gitoxide, never a subprocess; V1.md), consciously not now. Plus the standing candidates: chip ordering (agent `order` key + viewer drag) · mobile rail · `tailscale serve` SSE buffering check · unknown-class logging · tables and app subprocesses (grid candidate when reopened, author 2026-08-08: VisActor VTable, the pure-JS canvas core, UMD-vendorable, framework-neutral, async data model fitting reference-never-embed; AntV S2 the runner-up; Glide Data Grid is React-only, disqualifying; and embedding a live sqlnow session may beat a native block for the explore case) · sv-tree (author, 2026-08-08, post-mermaid: a nested markdown list *is* a tree; comrak parses it, CSS draws connector elbows, no layout engine, no deps; degrades to a readable indented list on old binaries. Sequence diagrams are the plausible second resident, lanes and rows, no graph layout; real DAG layout stays extension territory) · comment image attachments (author, 2026-08-09: paste or drop into the compose card, the daemon writes a project file, the comment stores the path; reference-never-embed extends to conversation; thumbnail in the card, /f/ already serves it, watch events name the path and the agent reads it with its file tool; genuine vision, pull not push, per the founding thesis; 'add it to the document' is one ask, per retired promotion) · resizable side rails (author, 2026-08-09, filed for v3 from the desktop pass: drag-to-resize both the contents rail and the comment bar, widths remembered as viewer preference, same law as their folds) · a --project flag or louder store identity in CLI output (2026-08-09, from the agent's own ergonomics report: store-from-cwd is elegant for humans, hazardous for agents whose shells reset; it caused two live cross-project misfires, one saved only by the comments FK) · patch-in-place block rendering (would restore native scroll anchoring; the client-side reading anchor covers the symptom for now) · per-line comments *inside code blocks* (author, 2026-08-07, filed as a comment on the models block itself: the diff `l:` fingerprint machinery generalized to any pre). All deferred twice; reopening them is a deliberate act.
</sv-prose>
...
and it released one resident early: mermaid left core the moment the
extension principle existed to receive it. Reopening anything here remains
a deliberate act, exactly as written.
</sv-note>
...
Partially exercised live, 2026-08-08, ahead of the formal ritual: comment, watch, reply and resolve each ran end to end, and two features (code-block bubbles, author-as-role) were requested *through the page* and shipped the same hour. This note is itself the sv-note demonstration: canon, always visible, versioned with the plan.
</sv-note>
