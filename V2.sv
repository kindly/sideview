...
- Placement (author, 2026-08-07, revising the margin-mark idea after seeing it): the affordance trails the text, never the margin. Every commentable bit ends with a comment bubble — hover-revealed when empty, **persistent with the count inside where a thread lives** (open threads' comments only: resolved conversations live in the tail list, visibly, which keeps the visibility law honest). Click the bubble to comment, or to open the conversation — the popover opens below its anchor, flips above when the room is above, and clamps to the viewport with its own scrollbar: it never grows the page or drags the scroll (author's report, 2026-08-08). Mobile has no hover: tap the text to reveal the empty bubble, tap the bubble to comment.
