...
**Watched diffs and the git machinery** — re-sequenced out of v2 by the author (2026-08-07): proven possible, mechanism settled (gitoxide, never a subprocess — V1.md), consciously not now. Plus the standing candidates: chip ordering (agent `order` key + viewer drag) · mobile rail · the scroll-feel decision · `tailscale serve` SSE buffering check · unknown-class logging · tables and app subprocesses · per-line comments *inside code blocks* (author, 2026-08-07, filed as a comment on the models block itself — the diff `l:` fingerprint machinery generalized to any pre) — deferred twice; reopening them is a deliberate act.
</sv-prose>
