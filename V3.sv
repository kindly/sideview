...
- **Deletion splits in two.** `page rm` deletes the file, which is right for a page sideview owns and alarming next to `DESIGN.md`; imported pages get an unbind verb and `rm` refuses a foreign format outright.
