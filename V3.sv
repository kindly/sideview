...
- **Page management at scale** — a project with many pages outgrows the chip strip: each `<sv-page>` may declare a `category` (canon, beside `order` — for committed pages, grouping is plan-worthy so it lives in the file), imported pages get theirs from `.sideview.toml`, and **the strip's top level is categories, not pages** (author, 2026-08-10 — a category is itself a page, so the strip stays short however many documents a project grows). The category you are inside expands its pages beside it, so switching siblings stays one click; uncategorized pages appear directly, as today. `/c/<category>` is a category's own page; the wordmark opens **`/home`**, whose subject is the categories, with each one's pages listed beneath as the useful extra. **Built and dogfooded 2026-08-10**: this project's own eleven pages now live in four categories, its docs still markdown. *(pulled 2026-08-09, thread 32)*
</sv-prose>
